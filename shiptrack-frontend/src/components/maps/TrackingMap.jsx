...
function TrackingMap({
  ...
  useEffect(() => {
    ...
    const loadRemainingRoute = async () => {
      try {
        ...
        if (!routeFeature?.geometry) {
          throw new Error(
            "No road route was found from the current location to the destination."
          );
        }
        ...
        setRemainingDistance(routeFeature.properties?.distance ?? null);
        setRemainingDuration(routeFeature.properties?.time ?? null);
      } catch (error) {
        if (error.name === "AbortError") {
          return;
        }

        console.error("Unable to load remaining shipment route:", error);
        setRoutePositions([]);
        setRemainingDistance(null);
        setRemainingDuration(null);
        setRouteError(
          error.message ||
            "Unable to calculate the remaining route. Please try again."
        );
      } finally {
      ...
      <div className="shipment-map-wrapper">
        <div className="shipment-map-header">
          <div>
            ...
            <p>
              The tracking map will appear after shipment location coordinates
              become available.
            </p>
          </div>
        </div>
        ...
        <div className="shipment-map-empty-state">
          ...
          <h3>Shipment location is not available</h3>
          <p>
            The logistics operator has not added the shipment location yet.
          </p>
        </div>
      </div>
    );
    ...
    <div className="shipment-map-wrapper">
      <div className="shipment-map-header">
        <div>
          ...
          <h2>Shipment Tracking</h2>
          <p>
            Follow the current shipment position and its remaining road route.
          </p>
        </div>
        ...
        <div className="shipment-map-summary">
          ...
          <div>
            ...
            <strong>
              {remainingDuration
                ? calculateArrivalTime(remainingDuration)
      ...
      <div className="shipment-map-legend">
        ...
        <div>
          ...
          <div>
            <strong>Remaining Route</strong>
            <small>
              {routeLoading
                ? "Calculating route..."
                : routePositions.length > 0
                  ? `${formatDistance(remainingDistance)} • ${formatDuration(
                      remainingDuration
                    )}`
