...
const RECIPIENT_GROUPS = [
  { role: "CUSTOMER", label: "Customers", optionsKey: "customers" },
  {
    ...
    optionsKey: "businessClients",
  },
  {
    role: "LOGISTICS_OPERATOR",
    label: "Logistics Operators",
    optionsKey: "logisticsOperators",
  },
  {
    role: "SUPPORT_AGENT",
    label: "Support Agents",
    optionsKey: "supportAgents",
  },
  { role: "DRIVER", label: "Drivers", optionsKey: "drivers" },
];
...
export default function SendNotification() {
  ...
  const idFor = (role, option) =>
    role === "CUSTOMER" ? option.customerId : option.userId;
  ...
  return (
    <form className="sendnotif-card" onSubmit={handleSend}>
      <p className="sendnotif-intro">
        Push a notification straight to a group's feed (and their email/SMS, if
        they have those channels enabled) — or tick a group open to pick
            ...
            <div className="sendnotif-picker-list">
              {optionsForRole(expandedRole).length === 0 ? (
                ...
                optionsForRole(expandedRole).map((option) => {
                  ...
                  const checked = (selectedIds[expandedRole] || []).includes(
                    id,
                  );
                  return (
      ...
      <div className="sendnotif-section">
        <label className="sendnotif-label">
          Related tracking ID (optional)
        </label>
        <input
          type="text"
          value={trackingId}
